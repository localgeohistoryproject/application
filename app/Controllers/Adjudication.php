...
class Adjudication extends BaseController
{
    ...
    public function view(int|string $id): void
    {
        ...
        if (count($query) !== 1) {
            ...
            echo view('reporter/table', ['query' => $AdjudicationSourceCitationModel->getByAdjudication($id), 'hasLink' => true, 'title' => lang('Application.reporterLinks')]);
            $EventModel = new EventModel();
            echo view('event/table', ['query' => $EventModel->getByAdjudication($id), 'title' => lang('Application.eventLinks'), 'eventRelationship' => true]);
